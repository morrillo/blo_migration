...
class ResCompany(models.Model):
    ...
    def _prepare_invoice_line(self, invoice_line):
        product = self.env['product.product'].search([('original_id','=',invoice_line['product_id'])])
        if not product:
            raise ValidationError('There is no product defined for original ID %s'%(invoice_line['product_id']))
    ...
    def migrate_invoices(self):
        ...
        if not dbname or not host or not username or not pwd:
            raise ValidationError('One of the connection parameters is missing')
        ...
        invoice_ids = models.execute_kw(db,uid,pwd,'account.invoice','search',[[('state','in',['open','paid'])]])
        for invoice_id in invoice_ids:
            invoice_data = models.execute_kw(db,uid,pwd,'account.invoice','read',[invoice_id])
            invoice_data = invoice_data[0]
            journal_id = self.env['account.journal'].search([('original_id','=',invoice_data['journal_id'][0])])
            if not journal_id:
                raise ValidationError('There is no journal for original_ID %s'%(invoice_data['journal_id'][0]))
            partner_id = self.env['res.partner'].search([('original_id','=',invoice_data['partner_id'][0])])
            if not partner_id:
                raise ValidationError('There is no partner_id for original_ID %s'%(invoice_data['partner_id'][0]))
            invoice_lines = []
            for invoice_line_id in invoice_data['invoice_line_ids']:
                line_data = models.execute_kw(db,uid,pwd,'account.invoice.line','read',[invoice_line_id])
            ...
            move_id = self.env['account.move'].search([('original_id','=',invoice_id)])
            if not move_id:
    ...
    def migrate_sql_invoices(self):
        ...
        if not dbname or not host or not username or not pwd:
            raise ValidationError('One of the connection parameters is missing')
        connect_string = "dbname='%s' user='%s' host='%s' password='%s'"%(dbname,username,host,pwd)
        try:
            conn = psycopg2.connect("dbname='%s' user='%s' host='%s' password='%s'"%(dbname,username,host,pwd))
        except:
            raise ValidationError("I am unable to connect to the database")
       
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        try:
            cur.execute("select * from account_invoice where type = 'out_invoice' and state in ('open','paid')")
        except:
            raise ValidationError('Problems querying the invoice table')
        rows = cur.fetchall()
        for invoice_data in rows:
            journal_id = self.env['account.journal'].search([('original_id','=',invoice_data['journal_id'])])
            partner_id = self.env['res.partner'].search([('original_id','=',invoice_data['partner_id'])])
            if not partner_id:
                raise ValidationError('There is no partner_id for original_ID %s'%(invoice_data['partner_id']))
            if not journal_id:
                raise ValidationError('There is no journal for original_ID %s'%(invoice_data['journal_id']))
            invoice_id = invoice_data['id']
            cur_lines = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            try:
                cur_lines.execute("select * from account_invoice_line where invoice_id = %s"%(invoice_id))
            except:
                raise ValidationError('Problems querying the invoice table')
            invoice_lines = []
